...
public class VideoController {
    ...
    public ResponseEntity<VideoResponseDTO> upload(
        ...
        // já devolve HLS assinado para o cliente atual (se possível)
        signDtoForRequest(dto, req);
    ...
    @GetMapping("/{id}")
    public VideoResponseDTO get(@PathVariable UUID id, HttpServletRequest req) {
        var dto = videoService.buscarPorId(id);
        return signDtoForRequest(dto, req);
    }

    /** Força refresh no Bunny e retorna DTO atualizado com URL assinada para o cliente atual */
    @GetMapping("/{id}/status")
    public VideoResponseDTO refresh(@PathVariable UUID id, HttpServletRequest req) {
    ...
    /** Assina a URL HLS considerando as regras (algoritmo/IP/TTL) e o cliente atual. */
    private VideoResponseDTO signDtoForRequest(VideoResponseDTO dto, HttpServletRequest req) {
        if (dto == null) return null;

        try {
            if (dto.getStreamVideoId() != null && !dto.getStreamVideoId().isBlank()) {
                dto.setHlsMasterUrl(streamUrlService.hlsUrlForRequest(dto.getStreamVideoId(), req));
            } else if (dto.getHlsMasterUrl() != null && !dto.getHlsMasterUrl().isBlank()) {
                dto.setHlsMasterUrl(streamUrlService.ensureSignedFromUrlForRequest(dto.getHlsMasterUrl(), req));
            }
        } catch (Exception e) {
